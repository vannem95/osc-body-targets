...
int main(int argc, char** argv) {
    ...
    std::filesystem::path simulation_model_path = 
        runfiles->Rlocation("mujoco-models/models/walter_sr/scene_walter_sr_updated_noda.xml");
    ...
    while(current_time < simulation_time) {
        ...
        double shin_rot_vel = 0.1*8.0*1.0*1.0; 

        double phase_offset = 3.14159 / 2.0;
        ...
        // ---------------------------------------------------------------------------------------------------------------------------------------------
        double shin_kp = 1.0*1000.0*1.2; 
        double shin_kv = 0.5*400.0*1.2;
        // ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        double thigh_height_increase_stairs = -0.005;
        ...
        // Explicitly calculate potential and kinetic energy
        mj_energyPos(mj_model, mj_data); // Populates mj_data->energy[0]
        mj_energyVel(mj_model, mj_data); // Populates mj_data->energy[1]

        logger.log("total_energy", mj_data->energy[0] + mj_data->energy[1]);

        double total_energy = mj_data->energy[0] + mj_data->energy[1];
        
        // --- LIVE TERMINAL READOUT (Throttled to run roughly 10 times a second) ---
        static int energy_print_counter = 0;
        if (energy_print_counter++ % 100 == 0) {
            std::cout << "\r[Time: " << std::fixed << std::setprecision(2) << current_time << " s] "
                      << "Total Energy: " << std::scientific << std::setprecision(4) << total_energy << " Joules   " 
                      << std::flush;
        }
        // --------------------------------------------------------------------------        
