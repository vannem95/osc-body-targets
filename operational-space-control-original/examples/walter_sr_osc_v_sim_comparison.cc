...
int main(int argc, char** argv) {
    ...
    while(current_time < simulation_time) {
        ...
        // 1. Declare them in the main scope
        // double thigh_lin_kp; 
        // double thigh_lin_kv;

        // // 2. Assign values without the word 'double'
        // if (current_time > 1.8 && current_time < 2.2) {
        //     thigh_lin_kp = 0.0; 
        //     thigh_lin_kv = 0.0;
        // } else {
        //     thigh_lin_kp = 100.0*5.0; 
        //     thigh_lin_kv = 20.0*5.0; 
        // }
        ...
        // =====================================================================
        // PROOF: PLANNED VS ACTUAL JOINT ACCELERATION
        // =====================================================================
        // 1. Extract the planned joint accelerations (dv) from the QP solution
        auto dv_planned = solution(Eigen::seqN(0, optimization::dv_size));

        // 2. Get the exact DOF index for the Head Right Shin (Joint 8 in your XML)
        int hr_shin_dof = mj_model->jnt_dofadr[8]; 
        
        // 3. Log the QP's planned acceleration vs MuJoCo's actual physics acceleration
        ...
        // =========================================================
        // LOG THE PD DEMAND PROOF (HEAD RIGHT)
        // =========================================================
        logger.log("hrh_pd_demand_z", hrh_linear_control);
        // logger.log("hrf_fz_plan", hrf_fz);
        // =========================================================

 

        // -------------------------------------------------------------------------------------
        // EXTRACT CONTACT FORCES (All 8 Wheels - 3D Forces)
        // -------------------------------------------------------------------------------------
        std::vector<std::string> wheel_names = {"tl_f", "tl_b", "tr_f", "tr_b", "hl_f", "hl_b", "hr_f", "hr_b"};
        std::vector<double> actual_fx(8, 0.0);
        std::vector<double> actual_fy(8, 0.0);
        std::vector<double> actual_fz(8, 0.0);

        // Single pass over all contacts to accumulate actual 3D forces for all wheels
        for (int i = 0; i < mj_data->ncon; ++i) {
            int geom1 = mj_data->contact[i].geom[0];
            int geom2 = mj_data->contact[i].geom[1];
            
            for (int w = 0; w < 8; ++w) {
                if (geom1 == wheel_geom_ids[w] || geom2 == wheel_geom_ids[w]) {
                    double f_contact[6];
                    // Extract 6D force:torque in the contact frame
                    mj_contactForce(mj_model, mj_data, i, f_contact);
                    
                    // MuJoCo Contact Frame: [0] is Normal (Z), [1] and [2] are Tangential Friction (X/Y)
                    actual_fz[w] += std::abs(f_contact[0]); 
                    actual_fx[w] += f_contact[1];
                    actual_fy[w] += f_contact[2];
                }
            }
        }        
        ...
        // Head right front - controller vs sim
        // -------------------------------------------------------------------------------------
        // LOG ALL 8 WHEEL 3D FORCES (Controller vs Sim vs Limit)
        // -------------------------------------------------------------------------------------
        for (int w = 0; w < 8; ++w) {
            // The QP stacks 3 variables (Fx, Fy, Fz) per wheel starting at u_idx
            double plan_fx = solution[optimization::u_idx + (3 * w) + 0];
            double plan_fy = solution[optimization::u_idx + (3 * w) + 1];
            double plan_fz = solution[optimization::u_idx + (3 * w) + 2];
            
            // Log Planned Forces
            logger.log(wheel_names[w] + "_fx_plan", plan_fx);
            logger.log(wheel_names[w] + "_fy_plan", plan_fy);
            logger.log(wheel_names[w] + "_fz_plan", plan_fz);
            
            // Log Actual Forces (From MuJoCo physics)
            logger.log(wheel_names[w] + "_fx_actual", actual_fx[w]);
            logger.log(wheel_names[w] + "_fy_actual", actual_fy[w]);
            logger.log(wheel_names[w] + "_fz_actual", actual_fz[w]);
            
            // Log Soft Switch Limits (Applied to Z)
            logger.log(wheel_names[w] + "_fz_limit", current_force_limits[w]);
        }        // -------------------------------------------------------------------------------------
        // APPLY CONTROL & LOG
        // -------------------------------------------------------------------------------------
        
        // 1. Get the site ID for the active trailing wheel
        int hrf_site_id = mj_name2id(mj_model, mjOBJ_SITE, "hrf_wheel_site"); 

        // 2. Ask MuJoCo to calculate the 6D velocity of the site
        double site_vel[6];
        // The '0' at the end means we want global/world coordinates, not local.
        mj_objectVelocity(mj_model, mj_data, mjOBJ_SITE, hrf_site_id, site_vel, 0);

        // 3. Log the actual vertical velocity of the wheel
        // MuJoCo returns [angular_x, angular_y, angular_z, linear_x, linear_y, linear_z]
        // Therefore, the global Z linear velocity is at index 5.
        logger.log("wheel_vel_z_actual", site_vel[5]);

        // 4. Log the solver's assumed velocity
        // The rigid-body equality constraint assumes this is strictly 0.0.
        logger.log("wheel_vel_z_plan", 0.0);



        // ================================================================================================================== 
        //  Contact force discrepancy proof
        // ================================================================================================================== 
        // 1. Isolate the 10N deficit on the Z-axis of the active wheel (target_wheel_idx = 6)
        double fz_plan = solution[optimization::u_idx + (3 * target_wheel_idx) + 2];
        double fz_actual = actual_fz[target_wheel_idx]; 
        double fz_deficit = fz_actual - fz_plan;

        // 2. Get the Jacobian for just this specific wheel site directly from MuJoCo
        // (Re-using hrf_site_id from Proof 1)
        ...
        mj_jacSite(mj_model, mj_data, jacp.data(), jacr.data(), hrf_site_id);

        // 3. Map the Z-axis force deficit into Joint Torque
        // The 3rd row of jacp (index 2) corresponds exactly to the global Z-axis
        Vector<model::nv_size> missing_torque = jacp.row(2).transpose() * fz_deficit;

        // 4. Reconstruct the Mass Matrix locally in main.cpp
        ...
        // 5. Calculate the predicted Acceleration Error
        // Using Eigen's LLT decomposition: M * delta_qacc = missing_torque
        Vector<model::nv_size> predicted_accel_error = M.llt().solve(missing_torque);

        // 6. Calculate the REAL Acceleration Error at the Knee
        // int hr_shin_dof = mj_model->jnt_dofadr[8]; 
        double real_accel_error = mj_data->qacc[hr_shin_dof] - dv_planned(hr_shin_dof);

        // 7. Log them together for the correlation plot
